...
local SpawnNow = require(sharedFolder:WaitForChild("Utils"):WaitForChild("Thread"):Clone()).SpawnNow
local Promise = require(sharedFolder:WaitForChild("Utils"):WaitForChild("Promise"):Clone())
...
function Aero:RegisterEvent(eventName)
	local event = self.Shared.Utils.Signal.new()
...
local function LoadService(serviceFolder, servicesTbl)
	...
	for _,v in ipairs(serviceFolder:GetChildren()) do
		if (v:IsA("RemoteEvent")) then
			local event = Aero.Shared.Utils.Signal.new()
